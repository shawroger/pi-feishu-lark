...
export class ConversationManager {
  ...
  private registryInitPromise: Promise<void> | undefined;
  ...
  private async ensureRegistry(): Promise<{ authStorage: AuthStorage; modelRegistry: ModelRegistry }> {
    if (this.authStorage && this.modelRegistry) {
      return { authStorage: this.authStorage, modelRegistry: this.modelRegistry };
    }
    if (!this.registryInitPromise) {
      this.registryInitPromise = (async () => {
        const auth = await discoverAuthStorage(getAgentDir());
        this.authStorage = auth;
        this.modelRegistry = new ModelRegistry(auth);
      })();
    }
  ...
  async selectModel(key: string, provider: string, modelId: string, onReply: (text: string) => Promise<void>) {
    const previous = this.previousTurn(key);
    const next = previous.then(async () => {
      const { modelRegistry } = await this.ensureRegistry();
  ...
  async getAvailableModels() {
    const { modelRegistry } = await this.ensureRegistry();
    return modelRegistry.getAvailable().sort((a, b) => {
  ...
  private async createSession(key: string): Promise<AgentSession> {
    ...
    try {
      ({ session } = await createAgentSession({
        ...
        model,
        sessionManager,
        systemPrompt: (defaultBlocks: string[]) => {
          const base = defaultBlocks.join("\n").trim();
          return base ? [...defaultBlocks, feishuExtra] : [feishuExtra];
        },
